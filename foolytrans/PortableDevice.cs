    ...
    public class PortableDevice
    {
        ...
        public void DownloadFile(PortableDeviceFile file, string saveToPath)
        {
            ...
            try
            {
                ...
                targetStream = new FileStream(Path.Combine(saveToPath, filename),
        ...
        public PortableDeviceFolder GetContents()
        {
            var root = new PortableDeviceFolder("DEVICE", "DEVICE");
                ...
                IPortableDeviceContent content;
                this._device.Content(out content);
                EnumerateContents(ref content, ref root);
            ...
            do
            {
                ...
                if (fetched > 0)
                {
                    ...
                    if (currentObject is PortableDeviceFolder)
                    {
                        ...
                        PortableDeviceFolder folder = (PortableDeviceFolder)currentObject;
                        if (currentObject.Name.Equals("WhatsApp"))
                        {
                            
                            EnumWhats(ref content,ref folder);
                            cancela = true;
                            return;
                        }
                        EnumerateContents(ref content,ref folder);
                    }
                }
            } while (fetched > 0);
        }
        ...
        private static void EnumWhats(ref IPortableDeviceContent content,
            ref PortableDeviceFolder parent)
        {
            ...
            do
            {
                ...
                {
                    var currentObject = WrapObject(properties, objectId);
                    if (currentObject.Name.Equals("Databases"))
                    {
                        PortableDeviceFolder folder = (PortableDeviceFolder)currentObject;
                        parent.Files.Add(currentObject);
                        GetFiles(ref content,ref folder);
                        return;
                    }
                    continue;
                    ...
                    if (currentObject is PortableDeviceFolder)
                    {
                        //EnumWhats(ref content,ref (PortableDeviceFolder)currentObject);
                    }
                }
            } while (fetched > 0);
        }

        private static void GetFiles(ref IPortableDeviceContent content,
           ref PortableDeviceFolder parent)
        {
